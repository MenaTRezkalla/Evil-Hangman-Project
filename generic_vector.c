...
Status generic_vector_assignment(GENERIC_VECTOR hLeft, GENERIC_VECTOR hRight)
{
        Generic_vector* pRight = (Generic_vector*)hRight;

        int i;

        while(generic_vector_is_empty(hLeft) == FALSE)
        {
                generic_vector_pop_back(hLeft);
        }

        for(i = 0; i < pRight->size; i++)
        {
                if(generic_vector_push_back(hLeft, pRight->data[i]) == FAILURE)
		{
			return FAILURE;
		}
        }
